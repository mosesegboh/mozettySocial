<?php 
	class User{
		...
		public function getFriendArray(){
			$username = $this -> user['username'];
			$query = mysqli_query($this->con, "SELECT friend_array FROM users WHERE username='$username'"); //selecting just what we need will make out project faster
			$row = mysqli_fetch_array($query);
		...
		public function didReceiveRequest($user_from){
			...
			$check_request_query = mysqli_query($this->con, "SELECT * FROM friend_requests
		...
		public function getMutualFriends($user_to_check){
			...
			$user_to_array_explode = explode(",",$user_to_check_array);

			foreach ($user_array_explode as $i) {
				foreach ($user_to_array_explode as $j) {
					if ($i == $j && $i != "") {
						$mutualFriends++;
					}
				}
